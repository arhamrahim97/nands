<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateConfidenceItemsetsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('confidence_itemset', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('riwayat_id')->default(0);
            $table->string('itemset');
            $table->bigInteger('support_count_A');
            $table->bigInteger('support_count_itemset');
            $table->double('support', 10, 4);
            // $table->double('support_persen', 10, 4);
            $table->double('confidence', 10, 4);
            $table->double('confidence_persen', 10, 4);
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('confidence_itemset');
    }
}
